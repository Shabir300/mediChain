...
import { z } from 'zod';
import { getPatientSummary } from '@/ai/flows/doctor-ai-summary';
...
export function DoctorAiSheet() {
  ...
  const onSubmit = async (data: PromptFormValues) => {
    ...
    // In a real app, you would fetch real patient data based on the prompt.
    // For this demo, we use mock data if the prompt mentions "summary".
    if (data.prompt.toLowerCase().includes('summary')) {
        try {
            const result = await getPatientSummary({
                ...
                condition: "Patient reports fever and sore throat.",
                currentMedicine: "Albuterol inhaler as needed for asthma."
            });
            const aiMessage: ChatMessage = { sender: 'ai', text: result.summary };
            setChatHistory(prev => [...prev, aiMessage]);
        } catch (error) {
            console.error('AI Doctor Assistant Error:', error);
            const errorMessage: ChatMessage = { sender: 'ai', text: "I'm sorry, I couldn't fetch the patient summary." };
            setChatHistory(prev => [...prev, errorMessage]);
        }
    } else {
        const aiMessage: ChatMessage = { sender: 'ai', text: "I can help with patient summaries. For example, try asking 'Give me a summary for the next patient.'" };
        setChatHistory(prev => [...prev, aiMessage]);
    }
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button className="fixed bottom-6 right-6 h-16 w-16 rounded-full shadow-lg" size="icon">
            ...
            <span className="sr-only">Open AI Doctor Assistant</span>
        </Button>
      </SheetTrigger>
      <SheetContent className="flex flex-col">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto rounded-md border bg-muted/50 p-4 my-4">
            {chatHistory.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center text-center text-muted-foreground">
                    ...
                    <p className='font-bold'>Your AI assistant for quick patient insights.</p>
                    <p className='text-xs mt-2'>Try "Give me a summary for the next patient".</p>
                </div>
            ) : (
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            render={({ field }) => (
                <FormItem>
                <FormLabel>Ask the AI Assistant</FormLabel>
                <FormControl>
                    <Textarea
                    placeholder="e.g., Give me a summary for the next patient."
                    {...field}
                    disabled={isLoading}
                    />
                </FormControl>
