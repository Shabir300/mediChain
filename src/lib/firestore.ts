...
const createCollection = <T extends { id: string }>(collectionName: string) => {
  const collectionRef = collection(db, collectionName) as CollectionReference<Omit<T, 'id'>>;

  return {
    collectionRef, // Expose the collection reference for custom queries
    ...
    update: (id: string, data: Partial<T>) => updateDoc(doc(collectionRef, id), data as any),
    delete: (id: string) => deleteDoc(doc(collectionRef, id)),
    query: async (field: keyof T, value: any): Promise<T[]> => {
